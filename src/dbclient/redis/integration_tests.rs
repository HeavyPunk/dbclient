...
fn test_int() {
    ...
    let result = redis.fetch(&FetchRequest {
        query: vec![QueryElement::RawQuery("GET test_int".to_string())],
        limit: 2,
    });
...
fn test_string() {
    ...
    let result = redis.fetch(&FetchRequest {
        query: vec![QueryElement::ListAllItemsFrom("tags".to_string())],
        limit: usize::MAX,
    });
...
fn test_hash() {
    ...
    let result = redis.fetch(&FetchRequest {
        query: vec![QueryElement::RawQuery("HGETALL user:1001".to_string())],
        limit: usize::MAX,
    });
